...
const TorrentPriorityCell = memo(
	({ torrentHash, fileId, priority }: { torrentHash?: string; fileId: string; priority: QBittorrentFilePriority }) => {
		...
		return (
			<Popover>
				<PopoverTrigger>
					<Link className="cursor-pointer" color="foreground" size="sm">
				...
				<PopoverContent className="p-3 w-full flex flex-row gap-3 items-center">
					...
					<Button
						title="Save new priority"
						isDisabled={!torrentHash}
						isIconOnly
						radius="sm"
						isLoading={isLoading}
...
const FileNameCell = memo(
	({
		...
		fileName: string
		onNamePressed: (target: HTMLElement, path: string, fullPath: string) => void
	}) => (
		<Breadcrumbs>
...
export function TorrentDetailsFiles({
	...
	const onNamePressed = useCallback(
		(target: HTMLElement, path: string, fullPath: string) => {
	...
	return (
		<>
			...
			>
				<TableHeader>
					<TableColumn width={50} allowsSorting key="index">
						INDEX
					</TableColumn>
					<TableColumn minWidth="100%" allowsSorting key="name">
						NAME
					</TableColumn>
					<TableColumn width={100} allowsSorting key="size">
						SIZE
					</TableColumn>
					<TableColumn width={100} allowsSorting key="progress">
						PROGRESS
					</TableColumn>
					<TableColumn width={50} allowsSorting key="availability">
						AVAIL.
					</TableColumn>
					<TableColumn width={150} allowsSorting key="priority">
						PRIORITY
					</TableColumn>
				</TableHeader>
				<TableBody items={sortedFiles || []}>
					{({ index, name, size, progress, availability, priority }) => (
						<TableRow key={index}>
							...
							<TableCell>
								<FileNameCell fileName={name} onNamePressed={onNamePressed} />
							</TableCell>
							<TableCell>{prettyBytes(size)}</TableCell>
							<TableCell>
								<TorrentProgressCell progress={progress} torrentState="unknown" />
							</TableCell>
			...
			<Popover
				...
				crossOffset={45}
				onClose={() => setIsRenamePopoverOpen(false)}
				triggerRef={popoverTargetRef}
			>
				<PopoverTrigger>
					<></>
				</PopoverTrigger>
					...
					<Input
						...
						endContent={
							<Button
								...
								onPress={() => setNewName(normalizeFileName(newName))}
							>
								<IconWand width={16} />
							</Button>
						}
