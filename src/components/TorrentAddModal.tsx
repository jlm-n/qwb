...
export const TorrentAddModal = memo(({ isOpen, onOpenChange }: Pick<ModalProps, 'isOpen' | 'onOpenChange'>) => {
	...
	return (
		<Drawer isOpen={isOpen} onOpenChange={onOpenChange} classNames={{ base: 'md:m-3 sm:rounded-none md:rounded-lg' }}>
			<DrawerContent>
				{(onClose) => (
					<>
						<Form onSubmit={handleSubmit} className="h-full">
							<DrawerHeader className="flex flex-col gap-1">Add a new torrent</DrawerHeader>
							<DrawerBody className="w-full">
								...
								<Divider />
								<Checkbox name="automaticManagement" defaultSelected={true}>
									Automatic torrent management
								</Checkbox>
								<Checkbox name="startNow">Start torrents</Checkbox>
								<Checkbox name="addToTop">Add to top of queue</Checkbox>
								<Checkbox name="sequentialDownload">Download in sequential order</Checkbox>
								<Checkbox name="firstLastPiecesFirst">Download first and last piece first</Checkbox>
								...
								<Select name="category" label="Category" isLoading={isLoadingCategories}>
								...
								<Select name="stopCondition" label="Stop condition" defaultSelectedKeys={['None']}>
									<SelectItem key="None">None</SelectItem>
									<SelectItem key="MetadataReceived">Metadata received</SelectItem>
									<SelectItem key="FilesChecked">Files checked</SelectItem>
								</Select>
								<Select label="Content layout" name="contentLayout" defaultSelectedKeys={['Original']}>
...
function formDataToCommonPayload(selectedTags: Selection, data: Record<string, FormDataEntryValue>): QBittorrentAddTorrentCommonInput {
	...
	return {
		...
		rename: 'renameTorrent' in data ? data.renameTorrent.toString() : undefined,
		root_folder: 'rootFolder' in data,
		savepath: 'saveLocation' in data ? data.saveLocation.toString() : undefined,
		seedingTimeLimit: 'seedingTimeLimit' in data ? Number(data.seedingTimeLimit) : undefined,
		sequentialDownload: 'sequentialDownload' in data,
		skip_checking: 'skipHashChecking' in data,
		tags: tags.length > 0 ? tags : undefined,
		upLimit: 'uploadSpeedLimit' in data ? Number(data.uploadSpeedLimit) : undefined,
	}
}
