		...
		return (
			<Modal isOpen={isOpen} onOpenChange={(isOpen) => !isOpen && onClose()}>
				<ModalContent>
					{(onClose) => (
						<>
							<ModalHeader className="flex flex-col gap-1">Move torrent and files</ModalHeader>
							<ModalBody>
								<Input isReadOnly className="opacity-50" label="Current name" value={currentName} />
								<Input isReadOnly className="opacity-50" label="Current location" value={currentLocation} />
								<Input
									isRequired
									label="New location"
									value={location}
