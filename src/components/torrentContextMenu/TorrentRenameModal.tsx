		...
		return (
			<Modal isOpen={isOpen} onOpenChange={(isOpen) => !isOpen && onClose()}>
				<ModalContent>
					{(onClose) => (
						<>
							<ModalHeader className="flex flex-col gap-1">Rename torrent</ModalHeader>
							<ModalBody>
								<Input isReadOnly className="opacity-50" label="Current name" value={currentName} />
								<Input isRequired defaultValue={currentName} label="New name" onChange={(e) => setName(e.target.value)} />
							</ModalBody>
							<ModalFooter>
