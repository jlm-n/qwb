...
export const AppAndServerSettingsButton = memo(
	({ className, appSettingsOnly = false }: { className?: string; appSettingsOnly?: boolean }) => {
		...
		return (
			<>
				<Button isIconOnly className={className} onPress={open}>
					<IconSettings />
				</Button>
				<Drawer
					isOpen={isOpen}
					...
					<DrawerContent>
						...
						{currentTab === 'qbittorrentSettings' && (
							<DrawerFooter>
								...
								<Button color="primary" onPress={handleSubmit} isLoading={isSetPreferencesLoading}>
									Save preferences
								</Button>
							</DrawerFooter>
						)}
