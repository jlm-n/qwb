...
import { Checkbox, Divider, Input, Link, Select, SelectItem, Textarea } from '@heroui/react'
...
export const WebUISettingsSection = memo(
	...
	>) => {
		...
		const [bypassAuthSubnetWhitelistEnabledState, setBypassAuthSubnetWhitelistEnabledState] = useState(
			bypass_auth_subnet_whitelist_enabled
		)
		...
		return (
			<>
				...
				<Input name="web_ui_port" label="WebUI listen port" defaultValue={web_ui_port?.toString()} type="number" />
				<Divider className="my-2" />

				<Checkbox name="web_ui_upnp" defaultSelected={web_ui_upnp}>
					Use UPnP / NAT-PMP to forward the WebUI port from my router
				</Checkbox>
				<Divider className="my-2" />

				<div className="flex gap-1">
					<Checkbox name="use_https" isSelected={useHTTPSEnabledState} onValueChange={setUseHTTPSEnabledState}>
				...
				<Checkbox name="bypass_local_auth" defaultSelected={bypass_local_auth}>
					Bypass authentication for clients on localhost
				</Checkbox>
				<Checkbox
					name="bypass_auth_subnet_whitelist_enabled"
					isSelected={bypassAuthSubnetWhitelistEnabledState}
					onValueChange={setBypassAuthSubnetWhitelistEnabledState}
				>
					Bypass authentication for clients in whitelisted IP subnets
				</Checkbox>
				<Textarea
					name="bypass_auth_subnet_whitelist"
					label="Authentication bypass subnet whitelist (comma separated liste)"
					isDisabled={!bypassAuthSubnetWhitelistEnabledState}
					defaultValue={bypass_auth_subnet_whitelist}
				/>
				<Input
				...
				<Checkbox
					...
					isSelected={alternativeWebUIEnabledState}
					onValueChange={setAlternativeWebUIEnabledState}
				>
					Use alternative WebUI
				</Checkbox>
				<Input
					name="alternative_webui_path"
					label="Alternative WebUI path"
					isDisabled={!alternativeWebUIEnabledState}
					defaultValue={alternative_webui_path}
				/>
				<Divider className="my-2" />

				<Checkbox name="web_ui_clickjacking_protection_enabled" defaultSelected={web_ui_clickjacking_protection_enabled}>
					Enable clickjacking protection
				</Checkbox>
				<Checkbox name="web_ui_csrf_protection_enabled" defaultSelected={web_ui_csrf_protection_enabled}>
					Enable Cross-Site Request Forgery (CSRF) protection
				</Checkbox>
				<Checkbox name="web_ui_secure_cookie_enabled" defaultSelected={web_ui_secure_cookie_enabled}>
				...
				<Checkbox
					...
					isSelected={hostHeaderValidationEnabledState}
					onValueChange={setHostHeaderValidationEnabledState}
				>
					Enable Host header validation
				</Checkbox>
				<Input
					name="web_ui_domain_list"
					label="Authorized Host domains (semicolon separated values)"
					isDisabled={!hostHeaderValidationEnabledState}
					defaultValue={web_ui_domain_list}
				/>
				<Divider className="my-2" />

				<Checkbox
					name="web_ui_use_custom_http_headers_enabled"
					isSelected={useCustomHTTPHeadersEnabledState}
					onValueChange={setUseCustomHTTPHeadersEnabledState}
				>
					Add custom HTTP headers
				</Checkbox>
				<Textarea
					name="web_ui_custom_http_headers"
					label="Custom HTTP headers (1 per line)"
					isDisabled={!useCustomHTTPHeadersEnabledState}
					defaultValue={web_ui_custom_http_headers}
				/>
				<Divider className="my-2" />

				<Checkbox
					name="web_ui_reverse_proxy_enabled"
					isSelected={useReverseProxyEnabledState}
					onValueChange={setUseReverseProxyEnabledState}
				>
					Enable reverse proxy support
				</Checkbox>
				<Input
					name="web_ui_reverse_proxies_list"
					label="Trusted proxies list"
					isDisabled={!useReverseProxyEnabledState}
					defaultValue={web_ui_reverse_proxies_list}
				/>
				<Divider className="my-2" />

				<Checkbox name="dyndns_enabled" isSelected={dyndnsEnabledState} onValueChange={setDyndnsEnabledState}>
					Update my dynamic domain nam
				</Checkbox>
				<Select
					name="dyndns_service"
					label="Dynamic DNS service"
					isDisabled={!dyndnsEnabledState}
					defaultSelectedKeys={dyndns_service !== undefined ? new Set([dyndns_service.toString()]) : new Set()}
				>
