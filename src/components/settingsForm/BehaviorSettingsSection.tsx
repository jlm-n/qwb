...
>) => {
	...
	return (
		<>
			...
			<Checkbox name="file_log_enabled" isSelected={logEnabledState} onValueChange={setLogEnabledState}>Enable log file</Checkbox>
			<Input isDisabled={!logEnabledState} name="file_log_path" label="Log file location" defaultValue={file_log_path} />
			<Checkbox isDisabled={!logEnabledState} name="file_log_backup_enabled" isSelected={logBackupEnabledState} onValueChange={setLogBackupEnabledState}>Backup log file after</Checkbox>
			<Input isDisabled={!logEnabledState || !logBackupEnabledState} name="file_log_max_size" label="Backup log file after" type="number" defaultValue={file_log_max_size?.toString()} endContent="KiB" />
			<Checkbox isDisabled={!logEnabledState} name="file_log_delete_old" isSelected={logDeleteOldEnabledState} onValueChange={setLogDeleteOldEnabledState}>Delete old backup files</Checkbox>
			...
			<Divider className="my-2" />
			<Checkbox name="performance_warning" defaultSelected={performance_warning}>Log performance warnings</Checkbox>
		</>
	)
})
