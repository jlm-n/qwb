		...
		return (
			<>
				...
				<Input
					...
					endContent={
						<Button size="sm" variant="light" title="Use random port" onPress={shuffleListenPort} isIconOnly>
							<IconArrowsShuffle width={20} />
						</Button>
					}
				/>
				<Checkbox name="upnp" isSelected={upnpEnabledSate} onValueChange={setUpnpEnabledState}>
				...
				<Checkbox name="i2p_enabled" isSelected={i2pEnabledState} onValueChange={setI2pEnabledState}>
					I2P (Experimental)
				</Checkbox>
				...
				<Checkbox name="i2p_mixed_mode" isDisabled={!i2pEnabledState} defaultSelected={i2p_mixed_mode}>
					Use I2P mixed mode
				</Checkbox>
				<Input
				...
				<Input
					name="proxy_port"
					label="Proxy Port"
					isDisabled={proxyTypeState === 'None'}
					defaultValue={proxy_port?.toString()}
					type="number"
				/>
				<Checkbox
					name="proxy_auth_enabled"
					isDisabled={proxyTypeState === 'None'}
					isSelected={proxyAuthEnabledState}
					onValueChange={setProxyAuthEnabledState}
				>
				...
				<Input
					...
					type="password"
					label="Proxy password (saved unencrypted)"
				/>
				<Checkbox name="proxy_hostname_lookup" isDisabled={proxyTypeState === 'None'} defaultSelected={proxy_hostname_lookup}>
					Use proxy for hostname lookups
				</Checkbox>
				<Checkbox name="proxy_bittorrent" isDisabled={proxyTypeState === 'None'} defaultSelected={proxy_bittorrent}>
					Use proxy for bittorrent
				</Checkbox>
				<Checkbox name="proxy_peer_connections" isDisabled={proxyTypeState === 'None'} defaultSelected={proxy_peer_connections}>
					Use proxy for peer and web seed connection
				</Checkbox>
				<Checkbox name="proxy_rss" isDisabled={proxyTypeState === 'None'} defaultSelected={proxy_rss}>
					Use proxy for RSS
				</Checkbox>
				<Checkbox name="proxy_misc" isDisabled={proxyTypeState === 'None'} defaultSelected={proxy_misc}>
					Use proxy for general purposes
				</Checkbox>
				<Divider className="my-2" />

				<Checkbox name="ip_filter_enabled" isSelected={ipFilterEnabledState} onValueChange={setIpFilterEnabledState}>
					Enable IP filtering
				</Checkbox>
				<Input
					name="ip_filter_path"
					isDisabled={!ipFilterEnabledState}
					defaultValue={ip_filter_path}
					label="Path to IP filter file (.dat, .p2p, .p2b files only)"
				/>
				<Checkbox name="ip_filter_trackers" isDisabled={!ipFilterEnabledState} defaultSelected={ip_filter_trackers}>
					Apply IP filtering to trackers
				</Checkbox>
				<Textarea
					name="banned_IPs"
					label="Manually banned IP addresses"
					isDisabled={!ipFilterEnabledState}
					defaultValue={banned_IPs}
				/>
			</>
		)
	}
