	...
	>) => {
		...
		return (
			<>
				<Input
					name="up_limit"
					defaultValue={up_limit?.toString()}
					label="Global upload limit (0 is unlimited)"
					type="number"
					endContent={<p className="text-nowrap">Bytes/s</p>}
				/>
				<Input
					name="dl_limit"
					defaultValue={dl_limit?.toString()}
					label="Global download limit (0 is unlimited)"
					type="number"
					endContent={<p className="text-nowrap">Bytes/s</p>}
				/>
				...
				<Input
					name="alt_up_limit"
					defaultValue={alt_up_limit?.toString()}
					label="Alternative upload limit (0 is unlimited)"
					type="number"
					endContent={<p className="text-nowrap">Bytes/s</p>}
				/>
				<Input
					name="alt_dl_limit"
					defaultValue={alt_dl_limit?.toString()}
					label="Alternative download limit (0 is unlimited)"
					type="number"
					endContent={<p className="text-nowrap">Bytes/s</p>}
				/>
				<Checkbox name="scheduler_enabled" isSelected={schedulerEnabledState} onValueChange={setSchedulerEnabledState}>
				...
				<div className="flex gap-3">
					...
					<TimeInput
						isDisabled={!schedulerEnabledState}
						value={new Time(scheduleFromHourState, scheduleFromMinState)}
						label="From (in the setup timezone)"
						onChange={onScheduleFromChangedCallback}
					/>
					...
					<TimeInput
						isDisabled={!schedulerEnabledState}
						value={new Time(scheduleToHourState, scheduleToMinState)}
						label="To (in the setup timezone)"
						onChange={onScheduleToChangedCallback}
					/>
				</div>
				<Select
					isDisabled={!schedulerEnabledState}
					...
					defaultSelectedKeys={scheduler_days !== undefined ? new Set([scheduler_days?.toString()]) : new Set()}
				>
				...
				<Checkbox name="limit_utp_rate" defaultSelected={limit_utp_rate}>
					Apply rate limit to µTP protocol
				</Checkbox>
				<Checkbox name="limit_tcp_overhead" defaultSelected={limit_tcp_overhead}>
					Apply rate limit to TCP overhead
				</Checkbox>
				<Checkbox name="limit_lan_peers" defaultSelected={limit_lan_peers}>
